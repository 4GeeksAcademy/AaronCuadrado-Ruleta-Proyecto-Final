...
#Ruta para obtener los datos de una reserva especifica
...
#Ruta para reservar mantenimiento para un vehiculo
@reservations.route('/bookings/<int:booking_id>/maintenance', methods=['PUT'])
def book_maintenance(booking_id):
    ...
    #Verificar si el usuario es el propietario de la reserva
    if booking.user_id != session['user_id']:
        return jsonify({"error": "No puedes reservar mantenimiento para una reserva que no te pertenece"}), 403
    
    #Reservar mantenimiento
    booking.maintenance_booked = True
    db.session.commit()

    return jsonify({"message":"Mantenimiento reservado con exito", "booking": booking.serialize()}), 200
